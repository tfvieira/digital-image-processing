...
planes[0] = norm_img(planes[0])
planes[1] = norm_img(planes[1])
...
while cv2.waitKey(1) != ord('q'):
    ...
    print('time = ', str(1/fps), ' fps = ', str(fps))
...
while 0xFF & cv2.waitKey(1) != ord('q'):
    ...
    ifft_complex_filtered = np.fft.ifft2(fft_complex_filtered)
    ifft_real_filtered = np.real(ifft_complex_filtered)
    ifft_real_filtered = 255 * norm_img(ifft_real_filtered)
...
#                            float(2 * np.pi * theta/100.0)) # ~0.2 second
    ...
    print('Elapsed time (seconds) = ', str(t))
...
while cv2.waitKey(1) != ord('q'):
    ...
    cv2.imshow('img', cv2.applyColorMap((255 * norm_img(img)).astype(np.uint8), 
                                        cv2.COLORMAP_COOL))
    cv2.imshow('mag', cv2.applyColorMap(np.fft.fftshift((255 * norm_img(mag)).astype(np.uint8)), 
                                        cv2.COLORMAP_COOL))
cv2.destroyAllWindows()
